    ...
    public partial class Marquee : MonoBehaviour {
        ...
        private IEnumerator RunContinuous(int runId) {
            ...
            while (runId == m_RunId) {
                ...
                if (m_LayoutDirty || layoutDirection != Direction || !Mathf.Approximately(layoutSpacing, Spacing)
                    || !Mathf.Approximately(layoutSegmentSpacing, SegmentSpacing)) {
                    m_LayoutDirty = false;
        ...
        private Vector2 ApplySegments(MarqueeUnit unit, MarqueeItemData item, int index) {
            ...
            for (var i = 0; i < segments.Count; i++) {
                ...
                if (renderer == null) {
                    Debug.LogWarning(
                        $"UIMarquee: 找不到可渲染片段「{seg.GetType().Name}」的渲染器（spine 片段需引入 UIMarquee.Spine 扩展并自动注册）。", this);
                    continue;
                }
            ...
            for (var i = 0; i < unit.Views.Count; i++) {
