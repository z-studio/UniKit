...
namespace PrimeTween {
    ...
    public readonly struct Easing {
        ...
        private Easing(ParametricEase type, float strength, float period = float.NaN) {
            ease = Ease.Custom;
            curve = null;
            parametricEase = type;
        ...
        public static Easing Curve([NotNull] AnimationCurve curve) => new(Ease.Custom, curve);

        /// <summary>Customizes the bounce <see cref="strength"/> of Ease.OutBounce.</summary>
        public static Easing Bounce(float strength) => new(ParametricEase.Bounce, strength);

        /// <summary>Customizes the exact <see cref="amplitude"/> of the first bounce in meters/angles.</summary>
        public static Easing BounceExact(float amplitude) => new(ParametricEase.BounceExact, amplitude);

        /// <summary>Customizes the overshoot <see cref="strength"/> of Ease.OutBack.</summary>
        public static Easing Overshoot(float strength) =>
            new(ParametricEase.Overshoot, strength * StandardEasing.kBackEaseConst);

        /// <summary>Customizes the <see cref="strength"/> and oscillation <see cref="period"/> of Ease.OutElastic.</summary>
        public static Easing Elastic(float strength, float period = StandardEasing.kDefaultElasticEasePeriod) {
            if (strength < 1) {
        ...
        internal static float EvaluateParametricEase(float t, ref TweenData rt, ref UnmanagedTweenData d) {
            ...
            if (parametricEase == ParametricEase.BounceExact) {
                ...
                float strengthFactor = fullAmplitude < 0.0001f ? 1f : 1f / (fullAmplitude * (1f - k_FirstBounceAmpl));
        ...
        private const float k_FirstBounceAmpl = 0.75f;
        ...
        private static float Bounce(float t, float strength) {
            ...
            float bounce() {
                if (t < 2 / d1) {
                    return n1 * (t -= 1.5f / d1) * t + k_FirstBounceAmpl;
                }
        ...
        public static float Evaluate(float interpolationFactor, Ease ease) {
            switch (ease) {
                case Ease.Custom:
                    Debug.LogError(
                        "Ease.Custom is an invalid type for Easing.Evaluate(). Please choose another Ease type instead."
                    );
