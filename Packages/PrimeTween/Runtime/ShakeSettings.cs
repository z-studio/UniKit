...
namespace PrimeTween {
    ...
    public struct ShakeSettings {
        internal const float kDefaultFrequency = 10f;

        [Tooltip(
            "Strength is applied per-axis in local space coordinates.\n\n"
        ...
        [Tooltip(
            "Falloff ease is inverted to achieve the effect of shake 'fading' over time. Typically, eases go from 0 to 1, but falloff ease goes from 1 to 0.\n\n"
            + "Default is Ease.Linear.\n\n"
            + "Set to "
            + nameof(Ease)
            + "."
            + nameof(Ease.Custom)
            + " to have manual control over shake's 'strength' over time."
        )]
        ...
        [Tooltip(
            "Represents how asymmetrical the shake is.\n\n"
            + "'0' means the shake is symmetrical around the initial value.\n\n"
            + "'1' means the shake is asymmetrical and will happen between the initial position and the value of the '"
            + nameof(strength)
            + "' vector.\n\n"
        ...
        /// <see cref="PrimeTweenManager.kDefaultShakeEase"/>
        ...
        internal ShakeSettings(
            Vector3 strength,
            float duration,
            float frequency,
            Ease? falloffEase,
            [CanBeNull] AnimationCurve strengthOverTime,
            ...
            if (falloffEase == Ease.Custom) {
                if (strengthOverTime == null || !TweenSettings.ValidateCustomCurve(strengthOverTime)) {
                    Debug.LogError(
                        $"Shake falloff is Ease.Custom, but {nameof(this.strengthOverTime)} is not configured correctly. Using Ease.Linear instead."
                    );
        ...
        public ShakeSettings(
            ...
            : this(
                ...
                duration,
                frequency,
                enableFalloff ? Ease.Default : (Ease?)null,
                null,
                easeBetweenShakes,
                asymmetryFactor,
                cycles,
        ...
        internal TweenSettings TweenSettings =>
            new(
                duration,
                Ease.Linear,
                cycles,
