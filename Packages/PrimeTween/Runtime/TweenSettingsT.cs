...
namespace PrimeTween {
    ...
    public struct TweenSettings<T> where T : struct {
        [Tooltip(Constants.kStartFromCurrentTooltip)]
        public bool startFromCurrent;

        [Tooltip(Constants.kStartValueTooltip)]
        public T startValue;

        [Tooltip(Constants.kEndValueTooltip)]
        public T endValue;
        ...
        public TweenSettings(
            ...
            : this(
                endValue,
                new TweenSettings(duration, ease, cycles, cycleMode, startDelay, endDelay, useUnscaledTime, updateType)
            ) { }
        ...
        /// <param name="startFromCurrent">If true (default), the animation will start from the current value. Useful to prevent abrupt changes in case the previous animation if still running.<br/>
        ...
        /// }
        /// </code></example>
        public readonly TweenSettings<T> WithDirection(bool toEndValue, bool startFromCurrent = true) {
            if (this.startFromCurrent) {
                Debug.LogWarning(
                    nameof(this.startFromCurrent)
                    + " is already enabled on this TweenSettings. The "
                    + nameof(WithDirection)
