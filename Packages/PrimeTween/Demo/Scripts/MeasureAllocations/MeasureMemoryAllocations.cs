...
namespace PrimeTweenDemo {
    public class MeasureMemoryAllocations : MonoBehaviour {
        ...
        [SerializeField]
        List<string> filterAllocations = new List<string>();

        [SerializeField]
        List<string> ignoreAllocations = new List<string>();
        ...
        int? calcGCAllocInFrame(int frameIndex) {
            ...
            const HierarchyFrameDataView.ViewModes viewMode =
                HierarchyFrameDataView.ViewModes.MergeSamplesWithTheSameName
                | HierarchyFrameDataView.ViewModes.HideEditorOnlySamples;

            using (var data = ProfilerDriver.GetHierarchyFrameDataView(
                       frameIndex,
                       0,
                       viewMode,
                ...
                while (stack.Count > 0) {
                    ...
                    foreach (var childId in childrenBuffer) {
                        var gcAlloc = (int)data.GetItemColumnDataAsSingle(
                            childId,
                            HierarchyFrameDataView.columnGcMemory
                        );
                        ...
                        if (logAllocations) {
                            print(
                                $"GC Alloc in frame {frameIndex}: {EditorUtility.FormatBytes(gcAlloc)}\n"
